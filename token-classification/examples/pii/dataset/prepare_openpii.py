...
# English only
python examples/pii/dataset/prepare_openpii.py --languages en

# Multiple languages
python examples/pii/dataset/prepare_openpii.py --languages en,fr,de

# Use a local snapshot already on disk
python examples/pii/dataset/prepare_openpii.py --source local --local-path data/raw/pii

# Different model tokenizer
python examples/pii/dataset/prepare_openpii.py --model xlm-roberta-base

# Only label the first subword of each entity (legacy HF recipe; default labels all)
python examples/pii/dataset/prepare_openpii.py --no-label-all-tokens
"""
...
from token_classification.shared.sources import DriveSource, HFSource, LocalSource
