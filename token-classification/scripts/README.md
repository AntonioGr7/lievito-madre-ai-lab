...
# 2. Train
python scripts/train_token_classification.py \
...
```bash
python examples/pii/dataset/prepare_nemotron_pii.py \
    --max-length 512 \
...
```python
from token_classification.serve import TokenClassificationPredictor
...
```bash
python -m token_classification.serve \
...
```bash
cp examples/pii/configs/pii_mbert.yaml \
   examples/pii/configs/ner_xlmr.yaml
```
...
Then run with `--config examples/pii/configs/ner_xlmr.yaml`.
...
from token_classification.dataset import tokenize_for_trainer
...
from token_classification.dataset import save_preprocessing_meta
...
If your label set differs from the default PII entities, edit `ENTITY_TYPES` in [token_classification/dataset.py](../token_classification/dataset.py) — `LABEL_NAMES` is derived from it as `["O"] + B-/I- pairs`.
