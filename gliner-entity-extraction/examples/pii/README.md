...
python examples/pii/dataset/prepare_openpii.py --out-dir data/processed/pii-gliner
# or:
python examples/pii/dataset/prepare_nemotron.py --out-dir data/processed/pii-gliner
...
```bash
python scripts/train_gliner.py \
    --config examples/pii/configs/smoke.yaml \
...
python scripts/train_gliner.py \
    --config examples/pii/configs/smoke_modern_bi.yaml \
...
python scripts/train_gliner.py \
    --config examples/pii/configs/a100_nemotron_generalist.yaml
```
...
Before trusting a fine-tune, check it actually beats an off-the-shelf model on the same test split. [`baseline_zeroshot.py`](../../scripts/baseline_zeroshot.py) evaluates a pretrained GLiNER **with no training** using the identical scoring, threshold tuning, and chunking, then prints a side-by-side delta against the fine-tune's `test_metrics.json`:
