...
`examples/pii/dataset/prepare_openpii.py` is the worked example targeting OpenPII. Use it as a template.
...
Copy `examples/pii/dataset/prepare_openpii.py` as a starting point and adapt it to your corpus. The simplest pattern:
...
from gliner_entity_extraction.shared.preprocessing import save_preprocessing_meta
...
```yaml
# examples/pii/configs/<your>.yaml
...
python scripts/train_gliner.py \
    --config examples/pii/configs/pii_gliner.yaml --resume
...
[`GLiNERPredictor`](../gliner_entity_extraction/serve.py) is the single inference entrypoint. It loads a full-FT save and a LoRA save through the same constructor, and applies the right optimisation stack for the detected hardware.

```python
from gliner_entity_extraction.serve import GLiNERPredictor
...
```bash
python -m gliner_entity_extraction.serve \
    outputs/pii_gliner/gliner_exp_01/final \
