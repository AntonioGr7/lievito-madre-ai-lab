...
by the pipelines project (`scripts/generate_bi_encoder_pairs.py`),
...
python examples/financial/dataset/download_sec_edgar.py

# 2. Full download — pass 0 (or a negative number) to lift the cap.
python examples/financial/dataset/download_sec_edgar.py --sample-size 0
...
def main() -> None:
    ...
    print(
        "Next: in the pipelines project, point the generator at it, e.g.\n"
        f"  python scripts/generate_bi_encoder_pairs.py \\\n"
        f"      --config examples/custom_pairs/configs/sec_edgar_pairs.yaml \\\n"
        f"      --input {out_path}"
    )
