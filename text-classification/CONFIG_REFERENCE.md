...
Every field accepted by a YAML config maps 1-to-1 onto a HuggingFace `TrainingArguments`, `from_pretrained`, or `DataCollator` parameter, loaded into the `TrainConfig` dataclass in `<package>/shared/config.py`.
