...
```python
from text_classification.serve import TextClassificationPredictor
...
```bash
python -m text_classification.serve \
...
```bash
cp examples/emotion/configs/emotion_bert.yaml \
   examples/emotion/configs/emotion_roberta.yaml
```

Then run with `--config examples/emotion/configs/emotion_roberta.yaml`.
...
Copy `examples/emotion/dataset/prepare_emotion.py`, point it at your data, and run it:
...
from text_classification.dataset import tokenize_for_trainer
...
from text_classification.dataset import save_preprocessing_meta
