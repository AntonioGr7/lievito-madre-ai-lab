...
Package: [bi_encoder_dataset/](../bi_encoder_dataset/)
...
`(anchor, positive)` shape that the `embedding-bi-encoder` project's
[`scripts/train_bi_encoder.py`](../../../../embedding-bi-encoder/scripts/train_bi_encoder.py)
already consumes.
...
The pair dataset drops straight into the existing bi-encoder workflow. The
commands below run from the separate `embedding-bi-encoder` project (point its
`--processed-dir` at the dataset this pipeline wrote):
...
[examples/custom_pairs/configs/sec_edgar_pairs.yaml](../examples/custom_pairs/configs/sec_edgar_pairs.yaml)
...
└── shared/
    └── preprocessing.py     # vendored copy of the lab's preprocessing helpers

scripts/
└── generate_bi_encoder_pairs.py   # CLI entry point
```

A worked end-to-end example (download corpus → run pipeline → produce a
`(query, document)` DatasetDict) lives with its downstream consumer in the
`embedding-bi-encoder` project under `examples/custom_pairs/`.
